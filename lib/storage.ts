...
export function exportBudgetAsCSV(inputs: BudgetInputs): string {
  ...
  lines.push(
    ...
    sep,
    section('HEALTH, MEDICAL & INSURANCE'),
    row('Health Insurance', inputs.healthInsurance),
    row('Dental Insurance', inputs.dentalInsurance),
    row('Vision Insurance', inputs.visionInsurance),
    row('Life Insurance', inputs.lifeInsurance),
...
export function exportBudgetQuickSummary(inputs: BudgetInputs): string {
  ...
  return [
    ...
    row('Health, Medical & Insurance', breakdown.totalHealth, breakdown.totalHealth * 12),
